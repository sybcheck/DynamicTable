...
public class PdfReportM1HeaderFooter extends PdfPageEventHelper {
    ...
    public void onEndPage(PdfWriter writer, Document document) {
        ...
        ColumnText.showTextAligned(cb, Element.ALIGN_CENTER, footer, (document.rightMargin() + document.right() + document.leftMargin() - document.left() - len) / 2.0F + 190F, document.bottom() - 20, 0);

        // 6.写入页脚2的模板（就是页脚的Y页这俩字）添加到文档中，计算模板的和Y轴,X=(右边界-左边界 - 前半部分的len值)/2.0F + len ， y 轴和之前的保持一致，底边界-20
        cb.addTemplate(total, (document.rightMargin() + document.right() + document.leftMargin() - document.left()) / 2.0F + 190F, document.bottom() - 20); // 调节模版显示的位置
    ...
    public void onCloseDocument(PdfWriter writer, Document document) {
        ...
        String foot2 = " " + (writer.getPageNumber() - 0) + " 页";
